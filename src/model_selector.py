...
class ModelSelector:
    ...
    def select(
        ...
        Args:
            ...
            preferred_provider: Optional preferred provider to use first
    ...
    def trigger_circuit_breaker(
        self, provider_name: str, model_name: str, duration_sec: int = 300
    ):
        ...
        if provider_name in self.providers:
            ...
            for model in provider.models:
                if model.model_name == model_name:
                    ...
                    self.logger.warning(
                        f"Circuit breaker triggered for {provider_name}/{model_name} for {duration_sec}s"
                    )
                    return
        self.logger.error(
            f"Could not find model {provider_name}/{model_name} to trigger circuit breaker."
        )
    ...
    def get_available_models(
        ...
        for prov_name, provider in self.providers.items():
            provider_data = {"provider": prov_name, "models": []}
            for model in provider.models:
        ...
        return {"models": available_models, "aggregate_limits": aggregate_limits}
        ...
        for prov_name, provider in self.providers.items():
            ...
            for model in provider.models:
                ...
                statuses[prov_name][model.model_name] = {
                    ...
                    "cooldown_remaining": max(0, int(model.cooldown_until - now))
                    if in_cooldown
                    else 0,
                }
        return statuses
    ...
    def refresh_registry(self, discovered_models: dict[str, list[str]]):
        ...
        for provider_name, model_list in discovered_models.items():
            ...
            if provider_name not in self.providers:
                self.logger.warning(
                    f"New provider detected: {provider_name}. Initializing..."
                )
            ...
            ranked_discovery = sorted(
                text_models, key=lambda m: (score_model(m), m), reverse=True
            )
            ...
            if pruned_count > 0:
                self.logger.info(
                    f"Pruned {pruned_count} models for {provider_name} (no longer in top 10)."
                )
            ...
            for model_name in top_10_discovered:
                if model_name not in existing_names:
                    self.logger.info(
                        f"Adding new top-10 model for {provider_name}: {model_name}"
                    )
                    metadata = get_model_metadata(model_name)
                    new_tracker = ApiLimitsTracker(
                        provider_name,
                        model_name,
